...
description: Separate essential from accidental complexity and produce an evidence-first simplification report for a repo, module, or path.
---

<!-- Keep in sync with .claude/commands/simplicity_audit.md (slash-command original: same body; only frontmatter and
the Invocation lead-in differ). -->

# Simplicity Audit

Use this skill when the user asks for a simplicity audit, complexity audit, essential-vs-accidental complexity review,
or the simplest trustworthy design for a repository, module, feature, or path.
...
Use the user's prompt, including any text after `$simplicity-audit`, as the audit request:
...
consolidation with nothing to delete, hand it to the sibling `refactor-audit` skill (precedent: the
`session_op_layer_extraction` carve-out).
