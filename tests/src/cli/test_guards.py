...
class TestRequireRepoRoot:
    ...
    def test_not_in_repo_exits(
        ...
        monkeypatch.chdir(tmp_path)
        with patch(
            "forge.session.claude.paths.find_project_root", side_effect=FileNotFoundError("No git repository found")
        ):
            with pytest.raises(SystemExit) as exc_info:
                require_repo_root()
        assert exc_info.value.code == 1
...
class TestRequireMainRepoRoot:
    ...
    def test_in_child_worktree_exits(
        ...
        with (
            patch("forge.session.claude.paths.find_project_root", return_value=worktree_root),
        ...
        assert exc_info.value.code == 1
        captured = capsys.readouterr()
        assert "child worktree" in captured.out
    ...
    def test_subfolder_of_child_worktree_points_to_main_root(
        ...
        captured = capsys.readouterr()
        assert "child worktree" in captured.out
        assert "subdirectory" not in captured.out
    ...
    def test_git_not_found_allows(self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
        ...
        with (
            patch("forge.session.claude.paths.find_project_root", return_value=tmp_path),
            patch("forge.session.worktree.get_main_repo_root", side_effect=GitNotFoundError()),
        ):
            result = require_main_repo_root()
        assert result == tmp_path.resolve()
    ...
    def test_git_worktree_error_allows(self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
        ...
        with (
            patch("forge.session.claude.paths.find_project_root", return_value=tmp_path),
            patch("forge.session.worktree.get_main_repo_root", side_effect=GitWorktreeError("rev-parse", "failed", 1)),
        ):
            result = require_main_repo_root()
        assert result == tmp_path.resolve()
