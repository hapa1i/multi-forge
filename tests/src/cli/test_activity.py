...
def test_period_start_uses_local_calendar_boundaries() -> None:
    fixed_local = datetime(2026, 7, 3, 15, 30, tzinfo=timezone(timedelta(hours=-4)))

    assert activity_module._period_start("all") is None
    assert activity_module._period_start("today", now=fixed_local) == datetime(2026, 7, 3, 4, tzinfo=timezone.utc)
    assert activity_module._period_start("week", now=fixed_local) == datetime(2026, 6, 29, 4, tzinfo=timezone.utc)
    assert activity_module._period_start("month", now=fixed_local) == datetime(2026, 7, 1, 4, tzinfo=timezone.utc)
