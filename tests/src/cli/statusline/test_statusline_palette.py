...
from forge.cli import status_line as status_command
...
from forge.cli.statusline.formatting import get_context_display
...
def _render_with_statusline(**statusline_kwargs):
    ...
    with contextlib.ExitStack() as es:
        es.enter_context(patch.object(status_command, "_get_terminal_width", return_value=200))
...
class TestDefaultPaletteNoDrift:
    ...
    def test_representative_fields_match_constants(self):
        assert DEFAULT_PALETTE.path == fmt.GREEN_BOLD
        assert DEFAULT_PALETTE.branch == fmt.YELLOW_BOLD
        assert DEFAULT_PALETTE.breadcrumb == fmt.BREADCRUMB_COLOR
        assert DEFAULT_PALETTE.template == fmt.TEMPLATE_COLOR
        assert DEFAULT_PALETTE.think == fmt.BLUE
        assert DEFAULT_PALETTE.tier_opus == fmt.TIER_OPUS
        assert DEFAULT_PALETTE.ctx_low == fmt.CTX_LOW
        assert DEFAULT_PALETTE.ctx_crit == fmt.CTX_CRIT


class TestApplyPalette:
    def test_default_is_noop(self):
        text = f"{fmt.GREEN_BOLD}/x{fmt.RESET} {fmt.TIER_OPUS}[Opus]{fmt.RESET}"
    ...
    def test_earthy_remaps_path_and_tier(self):
        ...
        assert fmt.GREEN_BOLD not in out
        assert EARTHY_PALETTE.path in out
        assert fmt.TIER_OPUS not in out
        assert EARTHY_PALETTE.tier_opus in out
        assert fmt.RESET in out  # structural reset untouched
    ...
    def test_no_chained_substitution(self):
        ...
        text = f"{fmt.BLUE}T{fmt.RESET}"
        assert apply_palette(text, EARTHY_PALETTE) == f"{EARTHY_PALETTE.think}T{fmt.RESET}"

    def test_separators_untouched(self):
        # DARK_GRAY (SEP) is structural; earthy does not remap it.
        text = f"{fmt.DARK_GRAY}|{fmt.RESET}"
...
class TestEndToEnd:
    def test_default_keeps_constants(self):
        ...
        assert fmt.GREEN_BOLD in out  # path bold-green by default
        assert fmt.TIER_OPUS in out  # opus tier blue by default
