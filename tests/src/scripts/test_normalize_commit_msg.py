...
@pytest.mark.parametrize(
    ("message", "expected"),
    [
        ("fix\U0001f525 message", "fix message"),
        ("fix \U0001f525message", "fix message"),
    ],
)
def test_normalize_preserves_word_boundary_for_adjacent_inline_emoji(message: str, expected: str) -> None:
    emoji_map, _ = mod.load_maps()

    assert mod.normalize(message, emoji_map, {}) == expected
