...
class TestRunParallel:
    ...
    def test_cancellation_reaps_child_registered_after_cleanup(self, mock_popen, mock_killpg, _getpgid):
        ...
        try:
            with patch("forge.core.invoker._lifecycle.threading", observed_threading):
                with patch("forge.core.invoker._lifecycle.as_completed", fake_as_completed):
                    with pytest.raises(KeyboardInterrupt):
    ...
    def test_cancellation_reaps_retry_child_registered_after_cleanup(self, mock_popen, mock_killpg, _getpgid):
        ...
        wd.start()
        try:
            with patch("forge.core.invoker._lifecycle.threading", SimpleNamespace(Lock=ObservedLock)):
                with patch("forge.core.invoker._lifecycle.as_completed", fake_as_completed):
                    with pytest.raises(KeyboardInterrupt):
                        ClaudeHeadlessInvoker().run_parallel([_req(label="w0")])
        finally:
...
class TestRun:
    @patch("forge.core.invoker._lifecycle.subprocess.Popen")
    def test_single_shot_success_surfaces_run_id(self, mock_popen):
        mock_popen.return_value = _mock_proc("out")
    ...
    def test_single_shot_timeout_kills_process_group(self, mock_popen, mock_killpg, _getpgid):
        ...
        assert any(call.args == (333, signal.SIGTERM) for call in mock_killpg.call_args_list)
