...
def test_resume_autoname_collision_preserves_winner_manifest(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """Audit second-pass (HIGH): a losing concurrent resume must not overwrite the winner's MANIFEST.

    The manifest is the name reservation (`SessionStore.create_exclusive`), so the loser is rejected
    before it can write to the colliding path. The winner is injected *between* the loser's name
    generation and its create_exclusive -- the only interleaving that still reaches the collision
    branch now that `_generate_resume_name` also consults manifests and otherwise sidesteps a
    winner that is already on disk.
    """
    project = tmp_path / "project"
    project.mkdir()
    ...
    def winner_lands_first(self: SessionStore, manifest) -> None:
        """Let the winner claim `parent-resumed` just before the loser tries to."""
        nonlocal collided
        if self._session_name == "parent-resumed" and not collided:
            collided = True
    ...
    assert collided is True, "the loser must actually reach the colliding create_exclusive"
    assert child.name.startswith("parent-resumed-"), "loser must retry under a fresh unique name"
    assert b"winner-uuid-xyz" in winner_store.manifest_path.read_bytes(), "winner's content survives, not the loser's"
    assert winner_store.read().name == "parent-resumed"
    assert SessionStore(str(project), child.name).read().name == child.name
