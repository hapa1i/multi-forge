...
- ``src/forge/core/reactive/cost_tracking.py::read_verb_logs``
...
@pytest.mark.parametrize("bad_line", _BAD_LINES)
def test_read_verb_logs_skips_non_object_line(bad_line: str) -> None:
    path = _shard("costs", "verbs")
    _append(path, json.dumps({"ts": _now_ts(), "verb": "keep"}))
    _append(path, bad_line)

    records = read_verb_logs()

    assert [r.get("verb") for r in records] == ["keep"]
