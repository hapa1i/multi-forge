...
Invoke this skill with an optional file, directory, or review instruction.
...
1. If model family is `openai`: {{forge:resource_loading:resources/code-openai.md}}
2. If model family is `gemini`: {{forge:resource_loading:resources/code-gemini.md}}
3. Otherwise: {{forge:resource_loading:resources/code.md}}

If model family lookup returns empty output, `anthropic`, or errors, treat it as the default family and immediately load
the default `resources/code.md` instruction. Do not probe multiple variants.

In v1, a direct-session model pin does not override family selection. A direct model in the default family still uses
the default review resource.
