...
from forge.session.models import AdoptionConfirmed, CodexConfirmed, SessionState
...
def find_adoptable_rollout(thread_id: str, cwd: Path) -> Path:
    ...
    Raises:
        AdoptError: On no match, a cwd mismatch, or more than one match in ``cwd``.
    """
    ...
    for path in matches:
        ...
        if recorded is None:
            # The head shape is not pinned across codex versions. An unverifiable
            # rollout is skipped rather than assumed local: binding the wrong
            # directory is the failure this check exists to prevent.
            _log.debug("Rollout %s records no cwd; not adoptable here", path)
            continue
    ...
    if not in_cwd:
        ...
        raise AdoptError(
            f"Codex thread '{thread_id}' has a rollout, but it records no launch directory, "
            f"so it cannot be verified as belonging to {cwd}"
        )

    if len(in_cwd) > 1:
        listed = ", ".join(str(p) for p in sorted(in_cwd))
        raise AdoptError(
            f"Codex thread '{thread_id}' matches {len(in_cwd)} rollouts in {cwd}: {listed}. "
...
def adopt_codex_session(ctx: ExecutionContext, plan: CodexAdoptPlan, *, name: str) -> str:
    """Bind a Forge session to the planned Codex thread; return the session name.
    ...
    def _mutate(m: object) -> None:
        ...
        m.confirmed.codex = CodexConfirmed(
            ...
            # last_run_at stays None: adoption did not run a turn, and
            # context_delivery stays None because no transfer context was delivered.
        )
    ...
    try:
        ...
        _rollback_codex_adoption(name, ctx=ctx, store=store)
        raise AdoptError(f"could not record the Codex binding for '{plan.thread_id}': {e}") from e
...
def _rollback_codex_adoption(name: str, *, ctx: ExecutionContext, store: SessionStore) -> None:
    """Undo adoption's own writes, and only those.

    Like the Claude arm, deliberately avoids ``SessionManager.delete_session``:
    adoption's input is user-owned state, and the convenient rollback reaches
    transcript cleanup. Nothing here touches ``$CODEX_HOME``.
    """
    ...
    try:
        ...
        _log.warning("Codex adopt rollback failed (index entry): %s", e)

    try:
        store.delete()
    except Exception as e:
        _log.warning("Codex adopt rollback failed (manifest): %s", e)
