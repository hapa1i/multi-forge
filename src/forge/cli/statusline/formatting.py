"""Status-line presentation, ANSI width, truncation, and wrapping."""
...
def tier_color(tier: str, runtime: status_types.ProxyRuntimeTruth | None) -> str:
...
def get_tier_display(runtime: status_types.ProxyRuntimeTruth | None) -> str | None:
    ...
    oc = tier_color("opus", runtime)
    sc = tier_color("sonnet", runtime)
    hc = tier_color("haiku", runtime)
...
def format_line_changes(lines_added: int, lines_removed: int) -> str | None:
...
def wrap_output(output: str, available: int) -> str:
    ...
    sep_visible_width = visible_width(_HARDENED_SEP)

    line1_parts = [segments[0]]
    line1_visible = visible_width(segments[0])
    split_idx = 1

    for i in range(1, len(segments)):
        seg_visible = visible_width(segments[i])
    ...
    line2_visible = visible_width(line2)
...
def render_categories(
    ...
    """Join the two real render buckets into the final plain status line.

    Where parts are concatenated directly (path + branch).
    Stream segments are separator-joined in configured order.
    """
    parts: list[str] = []
