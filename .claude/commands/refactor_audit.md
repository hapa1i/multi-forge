---
description: Find behavior-preserving refactoring and reorganization opportunities — structure that fights the code's change patterns — evidence-first.
argument-hint: '[scope path] [--quick | --focused | --full]'
---

# Refactor Audit

Sister command to `/simplicity_audit`. That audit asks whether complexity should exist at all; this one asks whether the
...
it to `/simplicity_audit`; when a simplicity finding turns out to be justified code in the wrong shape, it belongs here.

This command shares the sister's epistemic core — evidence-first, calibrated confidence, adversarial verification,
