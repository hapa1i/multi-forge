...
| 3 | Registration mechanics (user/proj x toml/json surfaces; matcher support) | 10, 20 |
...
```bash
./reproduce.sh              # headless set: 00 10 20 30 60 70 (~16-18 short turns)
...
`--dangerously-bypass-hook-trust` automatically (`need_trust_bypass`, override with `PROBE_BYPASS_TRUST=0/1`); on
`[INTERACTIVE-ONLY]` rerun 20/30/70's turns inside an interactive session per stage 50's pattern.

### Verdict vocabulary

- Stage 10: `[FIRES-HEADLESS]` | `[FIRES-HEADLESS-TRUST-GATED]` | `[INTERACTIVE-ONLY]` | `[NO-FIRE-UNCATEGORIZED]`
...
1. **Never touches the real `~/.codex`**: every stage runs under an isolated `CODEX_HOME` inside a mktemp tree
   (stage 40 keeps a persistent home under the capture root because trust state must survive its sub-steps; its
   `auth.json` copy is removed on exit). Escape hatch `PROBE_USE_REAL_CODEX_HOME=1` is operator-consent-only and
   mutates real trust state.
2. **Auth**: `~/.codex/auth.json` is copied 0600 into the disposable home and dies with it. Hook env captures elide
   the values of anything matching `KEY|TOKEN|SECRET|AUTH|PASSWORD` at capture time — before sanitization ever runs.
...
writes, `-o` last-message oracle). Full headless set ≈ 16–18 short turns; `all` adds ~5 turns + 2 interactive runs —
in total roughly one trivial conversation of ChatGPT-subscription quota. Re-run individual stages, not the world.

## Relationship to the decision record

Findings land as a dated Stage-A-style block + go/no-go table in
`docs/board/doing/runtime_abstraction/checklist.md` (Phase 6). Sanitized representative captures are promoted to
`tests/fixtures/codex/hooks/` with a provenance README (clone the `tests/fixtures/codex/README.md` structure); those
fixtures pin the future Codex hook adapter's parsers exactly as `exec_json_success.jsonl` pins
