...
#
# Run AFTER reproduce.sh, BEFORE copying anything out of the cache.
set -euo pipefail
...
scan "openrouter-key" '\bsk-or-v1-[A-Za-z0-9]{16,}'
scan "openai-style-key" '\bsk-[A-Za-z0-9_-]{16,}'
scan "bearer-token" 'Bearer [A-Za-z0-9._-]{8,}'
scan "jwt-ish" 'eyJ[A-Za-z0-9_-]{10,}'
scan "api-key-assign" '(OPENROUTER_API_KEY|OPENROUTER_PROVISIONING_KEY|API_KEY|ACCESS_TOKEN)=[^<]'
