...
frontier and tier-1 derive cache identity from the same canonical action representation while retaining their existing
plan, route, budget, effort, target-metadata, and throttle dimensions.
...
- Plan edits, checker route/model/budget/effort changes, and target metadata continue to invalidate tier-1 cache
  entries.
- Shadow sampling remains deterministic for one canonical action identity and freezes the full context needed for
  replay.
...
Avoid placing raw action content in logs or persisted cache keys; hash the canonical representation.
