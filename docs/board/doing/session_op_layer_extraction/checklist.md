...
- [x] **5b -- `SessionManager` (56 patches, 11 call sites).** Each CLI module constructs `SessionManager` directly;
...
**5b verification (2026-07-02):**
`rg -n 'patch\("forge\.cli\.session\.SessionManager|_sess\(\)\.SessionManager' tests src` returned empty; remaining
`_sess()` definitions/calls are only the 5c launcher seams in `session_lifecycle` and `session_fork`; parent patch lines
are **151**;
