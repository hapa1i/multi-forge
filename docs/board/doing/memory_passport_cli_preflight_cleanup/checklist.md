...
Phase 1 (characterization). Checklist revised through two review passes on 2026-07-16; review is complete and
...
stream placement. Existing partial coverage: incompatible-pin refusal for `track`
...
`upgrade`. Phase 1 completes the grid and hardens stream assertions:

- [ ] Rootless path for each of the four leaves: from a cwd with no `.forge/`, exit 1. For `track`, `show`, and
...
- [ ] Unsafe-path coverage for every leaf: harden the existing `track` and `upgrade` cases and add `show` and `remove`.
...
- [ ] Missing-file for `remove`: exit 1, `File not found: {path}`.
- [ ] `passport show` read-only exemption: with an incompatible `.forge/project.toml` pin, `show` still succeeds (exit
  0, passport rendered on stdout, stderr empty). This is the load-bearing read-vs-mutation distinction the resolver must
  make explicit.
- [ ] No-mutation assertions on every new `remove` error path: existing target bytes stay unchanged where a target
  exists; a missing target remains absent. This matches the existing `*_without_editing_doc` pattern.
- [ ] Stream placement asserted explicitly across the full grid rather than through mixed `result.output`:
...
- [ ] Upgrade's three reachable stage tips pinned exactly (rootless / unsafe / missing table above), so a generic shared
  tip cannot silently replace them.
- [ ] Existing missing-file wording drift preserved for this cleanup in one test, without treating it as a preferred
  product distinction: `track` says `File does not exist:`; `show`/`upgrade`/`remove` say `File not found:`.
- [ ] Precedence tests (which error wins) so extraction cannot reorder:
...
- [ ] Success path per leaf named from existing tests when ticking; harden those tests with explicit stdout/stderr
  assertions rather than adding parallel success cases.
- [ ] Assertion: full characterization set passes against unmodified `src/forge/cli/memory.py`.

## Phase 2: Extraction

- [ ] Add a private structured preflight failure (kind: `rootless` | `unsafe` | `missing`, plus the detail needed for
...
- [ ] Parameter is named `enforce_compatibility` (not `mutating`). When true, the resolver calls
...
- [ ] Existence check stays `Path.is_file()` exactly (a directory at the target must keep failing as missing, not be
  admitted by a generic `exists()`).
- [ ] Remove the dead `try/except ForgeOpError` wraps and upgrade's unreachable context-failure branch (resolved finding
  above); note the removal in the change-log entry.
- [ ] Repoint `track`, `passport show`, `passport upgrade`, `passport remove` through the resolver, preserving `track`'s
  pre-context flag validation order.
- [ ] Explicitly kept OUT of the resolver (assert by inspection when ticking):
...
- [ ] Assertion: every Phase 1 characterization test passes without modification after the repoint.
- [ ] Assertion: `test_cli_rich_tips_go_through_output_helpers` and `test_cli_rich_errors_go_through_print_error` stay
  green (no hand-rolled `Tip:` / `[red]Error:[/red]` introduced).
...
- [ ] `make pre-commit` clean on the code change.
- [ ] Integration decision recorded: this card changes CLI plumbing only (no hooks, session lifecycle, memory-writer
...
- [ ] `docs/developer/cli_style_guidelines.md` checked: no update expected because no observable output changes
  (dead-code removal excepted); if any output intentionally changed, update the guide and note it here.
- [ ] Compact entry added to `docs/board/change_log.md` (bug-fix/cleanup size, 5-10 lines; names the dead-wrap removal).
- [ ] Durable lessons (if any survive review) proposed via `.forge/memory/shadow_impl_notes.md`, not written directly to
  `impl_notes.md`.
