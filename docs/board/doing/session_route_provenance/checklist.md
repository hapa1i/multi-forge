...
- [x] M1's marked transaction appends `launch_preflight` and `run_started` before yielding to runtime launch code and
  already owns `launch_aborted` vocabulary.
...
- [ ] Pin strict manifest round trips for absent, valid, malformed, and unknown-field projections.
...
- [ ] Append one required routing commit immediately after the existing M1 authority commit seam and before child
  invocation.
...
- [ ] Mark the authority attempt as pre-invocation-aborted so cleanup clears active state without appending `run_ended`.
...
- [ ] Update `docs/cli_reference.md` §§1 and 2.2: add both terminal leaves to the session-management table and add
  `%session model show` to the shipped direct-command list/scope.
...
| D1 routing append failure                | Routing append exception after authority start             | Authority-only abort; active state cleared; no child                           | `tests/src/core/ops/test_session_authority_launch.py`                                                           |
| D1 projection failure                    | Atomic projection exception after route commit             | Routing then authority abort; exact payload; no child                          | `tests/src/core/ops/test_session_authority_launch.py`                                                           |
| D1 compensation failure                  | One or both compensating appends fail                      | Primary error retained; every compensation attempted and named; no child       | `tests/src/core/ops/test_session_authority_launch.py`                                                           |
