...
- [x] **D9 -- Runtime-error folding (deliberate Claude-visible change).** `_to_review_result` folds
  ...
  and set `error = outcome.stderr.strip() or outcome.stdout.strip() or "Runtime reported error"`. Codex normally takes
  the effective-stderr arm; Claude's parsed envelope text normally takes the stdout arm. Scope honesty: because
  `_JSON_IS_ERROR_RELIABLE = True`, this also changes today's Claude behavior for exit-0 `is_error` envelopes (they
  currently reach synthesis as `success=True` while the ledger says `error`). Characterize the current Claude behavior
  first, then flip with a regression test for both runtimes. This is the one intended exception to the "all-Claude
...
| Runtime-error folding (D9)      | codex stream exit 0 + `is_error`; Claude exit-0 `is_error` envelope | `success=False`; stdout/stderr preserved; error precedence is stderr, stdout, exact generic fallback; ReviewResult and usage status agree; synthesis excludes it                                                        | `tests/src/review/test_engine.py`                           |
