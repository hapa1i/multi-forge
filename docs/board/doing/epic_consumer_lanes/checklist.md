...
from the 2026-06-26 workweave/Avengers-Pro discussion -- originated in `proposed/` and is now active in `doing/subscription_exhaustion_failopen/`
...
(subscription-exhaustion fail-open) is now the active cursor** -- promoted to
`doing/subscription_exhaustion_failopen/` (2026-06-30, branch `subscription_exhaustion_failopen`). A 2026-06-30 code
sweep found **no structured codex exhaustion signal** (no headroom field; no HTTP rate-limit headers on the direct
`codex exec` path -- it is a subprocess with stdout-JSONL only), so T7 is scoped **supervisor-only** (the one
repeated-dispatch codex consumer; shadow-curation is one-shot) with post-hoc failure classification, a **Phase 0
signal-capture gate**, and a sticky degrade kept off the write-once `confirmed.consumer_lanes` binding. It is **awaiting
plan review** before implementation. The `core.llm` representation is
...
- [ ] Promote T1b/T6 from inline sketch to member cards when they become the cursor. **T4 done** ->
  ...
  workweave discussion) -> promoted to `doing/subscription_exhaustion_failopen/` (2026-06-30, active). **T1b done** -> `done/consumer_lane_binding/`
