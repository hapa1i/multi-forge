...
`design_appendix.md` §A.2.1 is synced. **T4** (Codex-exec supervisor lane) is the **next member to open** (next queued
execution target -- still `todo/`, no branch yet), fully unblocked (deps T1a+T2+T3 all landed), authored in
`todo/codex_exec_supervisor_lane/`. **T7** (subscription-exhaustion fail-open) -- the one new ticket from the 2026-06-26
...
| T4           | `todo/codex_exec_supervisor_lane/`           | todo     | T1a,T2,T3  | authored (next to open)    |
...
- [x] **Unsupported-lane failure mode** (T4). **Decided: catch + fail-open** (consistent with `proxy_not_found`;
  ...
  lane to "aligned" (design_workflows §1.2). Wiring tracked in `todo/codex_exec_supervisor_lane/`; full seam list: epic
...
- [ ] Promote T4/T5/T1b/T6 from inline sketch to member cards (shape proven; T1a+T3 landed). **T4 promoted** ->
  `todo/codex_exec_supervisor_lane/`; **T7** (new, from the workweave discussion) ->
