...
- [ ] Rewrite `stop_cmd` (`:905`) to `nargs=-1` targets + `--all`/`-a` + `--yes`/`-y` (mirror `proxy delete`): drop the
...
- [ ] Add helper `_stop_runtime_instance(instance: BackendInstance)`: register `instance.adapter_type`
  ...
  the operand string.
- [ ] Explicit-target resolution order (per target): (1) present in `store.read().backends` -> stop it; (2a) a
...
- [ ] `--all` reads `store.read().backends` **directly** (never `list_backends()` -- its prune side-effect would mutate
...
- [ ] Pidless (`pid is None`) entries under `--all`: unregister and report distinctly
...
- [ ] Guards: `stop <id> --all` -> exit non-zero (conflict); `stop` with no target and no `--all` -> exit non-zero with
...
- [ ] Bulk previews + prompts by default; single/multiple **explicit** stops keep the existing no-confirm behavior (only
...
- [ ] Rewrite `delete_cmd` (`:925`) to `(adapter, yes)` -- **remove `--port`**. **Assertion:**
  `delete litellm --port 4000` errors via Click "No such option" (exit 2); `delete --help` shows no `--port`.
- [ ] Before `_resolve_local_adapter_operand`, reject a **runtime instance id** operand present in
...
- [ ] Keep config deletion behavior: `delete <adapter>` still stops matching `f"{adapter}-*"` instances first, then
...
- [ ] Remote boundary unchanged: `delete openrouter` keeps returning the intentional no-local-config message
...
- [ ] Leave `start_cmd` (`:854`) resolving via `_resolve_lifecycle_operand` (source id / adapter+`--port`).
...
- [ ] Reflect the asymmetry in `start`/`stop` help one-liners: `start` = start a local **source/adapter config**; `stop`
...
- [ ] Define the three id-spaces **once** in the `backend()` group help (`:58-67`): **source id** (`openrouter` -- an
...
- [ ] Fix B1 trap 1 (group example teaches the wrong id): the group example uses `litellm` (an adapter) while
...
- [ ] Fix B1 trap 2 (`show`'s only example is an instance): add a **source** example to `show_cmd` (`:637-638`, e.g.
  `show openrouter`) alongside the existing `show litellm-4000`. **Assertion:** `show --help` shows one source + one
  instance example.
- [ ] Clarify `reconcile` `--request-id` help (`:1000`): keep the `<source-id>` reference but say where to find them
...
- [ ] **B1 trap 3 -- source-row dual-key JSON (RESOLVED: keep both, document).** Two source-row emitters ship
...
- [ ] Add the card's acceptance cases (table below). Use the existing registry-fixture pattern in that file; assert exit
...
- [ ] Clean-break guards: `stop litellm --port 4000` and `delete litellm --port 4000` both exit 2 (Click "No such
  option"). **Assertion:** both asserted.
- [ ] Obsolete-test check (`coding_standards.md` §5): a base grep confirms **no** existing test exercises
  `stop <adapter> --port` or a supported bare `stop <adapter>`, so nothing is obsoleted here -- but re-run the grep
  after the rewrite and update/delete anything left asserting the old shape. **Assertion:** `--port` in the stop/delete
  tests appears only in the clean-break exit-2 guards; the unchanged `start ... --port` cases remain.

## Phase 6 -- Docs sync + changelog

- [ ] `docs/cli_reference.md` "Model management" table: update the `backend stop` row (stop a **runtime instance** by
...
- [ ] `docs/design_appendix.md` §A.2.1 (the `forge model backend` operator-view paragraph): update the "`start` and
...
- [ ] `docs/end-user/proxy.md`: add a short "flush suspect local backend processes" note (`stop --all` after a
  credential change), paralleling the `proxy delete --all` guidance. **Assertion:** the operator recovery path is
  documented.
- [ ] `docs/board/change_log.md`: feature-completion entry (Goal / Key changes / Verification) at closeout, naming the
...
- [ ] Focused suite `uv run pytest tests/src/cli/test_backend_commands.py -q` green; `make pre-commit` clean.
- [ ] Integration: none expected (host CLI + registry file; no `claude -p`/Docker path). Confirm and record if that
  holds.
- [ ] `change_log.md` entry added (feature-completion size).
- [ ] Docs synced: `cli_reference.md`, `design_appendix.md` §A.2.1, `end-user/proxy.md`.
- [ ] cli_style index annotated: **B1 shipped** (strike the group-help rows folded here); note the remaining B1 metavar
