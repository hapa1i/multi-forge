...
- [ ] Add fail-first long- and short-file-selector regressions proving the conflict exits 2 before stdin or file reads.
- [ ] Preserve missing-selector, file-only, diff-only, JSON, fail-mode, target extraction, and evaluation controls.

## Phase 2 -- Implement and document

- [ ] Reject simultaneous `--file`/`-f` and `--diff` selectors with a Click usage error at the command boundary.
- [ ] Document that exactly one content source is required without changing either successful input path or output
  schema.

## Phase 3 -- Verify and publish

- [ ] Run focused policy-check/output/regression tests and targeted policy integration.
- [ ] Commit this card after O080 and before the vocabulary follow-up.
