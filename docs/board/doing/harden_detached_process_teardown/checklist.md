...
- [x] Run focused backend adapter/manager/CLI and Claude/Codex/grouped invoker tests (`124 passed`).
- [x] Run backend CLI integration (`8 passed`) and real-process group teardown integration (`2 passed`); disclose the
  credential-blocked Codex smoke attempt.
- [x] Run `make test-unit` (`8,972 passed`, one existing platform skip, 122 deselected) and `make test-regression`
  (`745 passed`).
