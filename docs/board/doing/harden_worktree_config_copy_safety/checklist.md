...
- [x] Activate only Wave 8 order 4 from pushed closeout `3f50012c` on `agent/harden-worktree-config-copy-safety`; keep
  orders 5--19 parked.
- [x] Add fail-first regressions proving cleanup preserves a tracked descendant and glob/directory traversal excludes
...
- [x] Run focused config-copy/cleanup unit/regression checks (33 passed), Docker worktree suites (34 passed), and the
  session worktree-create path (one passed, 23 deselected).
- [x] Run `make test-unit` (9,317 passed, one skip), `make test-regression` (948 passed), targeted Docker session
  fork/worktree cleanup coverage, and `make pre-commit`.
- [x] Verify documentation size (`design.md` 29,990; appendix 29,988 Opus-5 tokens), all 970 local board links,
  stale-lane references, and diff hygiene.
- [ ] Commit, push, and open an independent draft PR.
