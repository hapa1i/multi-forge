...
| Shared `FORGE_SESSION`               | Single session manifest per worktree in team mode. Hooks resolve via launch env vars or UUID index lookup — all land at the same manifest. `agent_id`/`agent_type` (v2.1.69) may provide per-role identity in hook events `[VERIFY]`, but the shared manifest constraint remains. |
