...
**Status**: Done. Shipped via PR #39 (squash `ab690ac9`) plus the review follow-up (shared local-instance display,
unknown-`proxy.source` warn). Activated by the telemetry epic after `upstream_downstream_ledgers` landed. Spun out of
the `openrouter_observability` investigation (2026-06-16) while reasoning about whether provider lifecycle evidence
