...
[`cli_style_ux_compliance`](../../done/cli_style_ux_compliance/card.md) **A1** shipped in PR #70
...
**help-only, no metavar rename**) because both edit `backend.py` help. The remaining cli_style rows shipped in PR #72.
...
- Distinct from, but coupled to, `done/cli_style_ux_compliance` B1/C2, which improves help/metavar clarity for the same
...
The broader `source id` wording problem belongs to `done/cli_style_ux_compliance` B1/C2: `ModelSource.id` is real
