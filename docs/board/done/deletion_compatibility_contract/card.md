...
| `LLMClientFactory.get_cache_status` / `clear_cache` | Delete                 | No production callers or documented operator surface; move any useful cache behavior assertions to live factory paths.                                                                                                                                                    |
...
| `IndexState.needs_reindex`                          | Keep and wire          | It is unused, but expresses the missing byte-change guard. Use it to stop re-extracting identical transcript snapshots; do not delete the intended optimization.                                                                                                          |
