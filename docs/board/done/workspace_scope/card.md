...
**Status**: Done. Slice 2 shipped via [PR #122](https://github.com/hapa1i/multi-forge/pull/122) on 2026-08-03
(`a5aee0a9`) after implementation and verification on `feat/workspace-scope-slice2`. Two precursors shipped on branch
...
manually-created linked worktrees group under `--scope workspace`. Slice 1 made the core session-grouping query correct
with no new surface; Slice 2 adds the `forge workspace worktrees` read surface joining `git worktree list` with the
session index (`status` deferred — see CLI / UX Sketch).
