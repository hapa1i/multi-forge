...
- On new tracking, an omitted `--intent` keeps the strategy default; an explicitly empty or whitespace-only raw option
  fails before synthesis or any write.
...
- Every successful passport mutation (`write`, `remove`, `upgrade`) preserves the target's existing filesystem mode
  while retaining atomic replace semantics.
...
- New tracking and upgrade reject logical basenames `index.md` and `log.md` before any official-doc or shadow write. A
  symlink whose resolved target has either reserved basename is also rejected so an alias cannot mutate a reserved file.
...
| AT-02 | Proposal preflight and envelope          | valid new flow; explicit empty/whitespace intent and invalid type on new flow; invalid writer on new and existing-passport flows | valid official gets envelope/passport and shadow gets neither; invalid input changes neither official nor shadow tree                             | `tests/src/cli/test_memory.py`                                                                                                               |
...
| AT-08 | Unsafe frontmatter mutation fails safely | list/string/int/bool/null roots, BOM/EOF-delimiter forms; empty/comment-only controls                                            | write/remove/track/upgrade fail unsupported inputs without modification or second block; controls remain writable                                 | `tests/src/session/test_passport.py`, `tests/src/cli/test_memory.py`, `tests/regression/test_bug_okf_nonmapping_frontmatter_double_block.py` |
