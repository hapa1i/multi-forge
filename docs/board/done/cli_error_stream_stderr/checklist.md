...
- [x] Flip `print_error` (`output.py:64`) and `print_error_with_tip` (`:69`) defaults to `err_console`. **Fixes 0
...
- [x] Fix `handle_session_error` (`output.py:108`): `out = _resolve(console)` ->
  `out = console if console is not None else err_console`, so its **11 bare call sites** (`main.py`, `session_fork.py`,
  `session_lifecycle.py`, `session_manage.py`, `session_resume_modes.py`) route the plain-error path to stderr; its tip
  already follows via `print_tip(console=out)` (D-scope: wrapper uses stderr). **Assertion:** bare
  `handle_session_error(<plain ForgeSessionError>)` writes error+tip to stderr, exit 1.
- [x] Confirm `handle_corrupt_state_error` (`:124`) and `handle_unreadable_state_error` (`:143`) already default to
...
- [x] AST/semantic sweep: for every `print_error`/`print_error_with_tip` call passing `console=console` (240,
  AST-identified -- NOT a line grep), drop the `console=console` arg so it uses the new stderr default. **Assertion:**
  saved AST guard reports 0 `print_error*(console=console)` calls.
- [x] **Audit individually**: the `console`-param helper functions (e.g. `backend.py:_exit_click_error/_show_source`,
  ...
  repoint to `console=err_console` rather than dropping the arg. **Assertion:** resolved by dropping the injected stdout
  console for error-only calls; `search.py`'s local `Console()` override was also removed; no error path in these
  helpers lands on stdout.
- [x] Leave the 90 `console=err_console` and 4 `console=out` sites untouched. **Assertion:** re-scan shows 90
  `console=err_console`, 4 `console=out`, 241 defaulted calls, and 0 `console=console`.

## Phase 3 -- JSON (1a) + red secho (1c) -- corrected anchors

- [x] Add `err=True` to the **13** `click.echo(json.dumps({"error"|"routing_error": ...}))` sites: `auth.py:358,495`,
...
- [x] Add `err=True` to the **4** red `secho` sites: `auth.py:191,396,505,508` (`auth.py:182` is already correct).
...
- [x] Extend `tests/src/cli/test_output_streams.py` to trip an **in-branch** `--json` *error* path (post-arg-parse, not
  pre-flight): stdout empty, error JSON on stderr, exit non-zero. Pick a command whose `--json` branch raises after
  parsing.
- [x] Guard the two default fixes: bare `print_error("x")` -> stderr, and bare `handle_session_error(<plain error>)` ->
  stderr. Locks Phase 1 against regression.
- [x] (Optional) repo-grep/AST meta-tests mirroring the falsifiable check + the `console=console` re-scan, so a
  re-introduced offender fails CI. Scanner saved as AST tests for error helpers, adjacent error continuations, JSON
  error echoes, and red `secho`.

## Phase 5 -- Verify + docs

- [x] Focused suite: `uv run pytest tests/src/cli -q`. Watch for tests asserting error text on `result.stdout` that must
  move to `result.stderr` (the handler-default + 240-sweep will shift many). **Assertion:** green; list every assertion
  moved stdout->stderr. Moved stream assertions in `test_guards.py`, `test_session_codex.py`, and
  `test_session_resume_review.py`; added direct stdout/stderr assertions for config/proxy continuation hints; normalized
  one Rich-wrapped extension-enable assertion.
- [x] `make pre-commit` clean.
- [x] Docs: compliance, not a behavior change -- no design-doc edit expected. Touch `cli_style_guidelines.md` only if it
  still calls the stdout/stderr JSON guard "planned/not yet wired" (Slice 07 wording). No design-doc edit needed.
- [x] `change_log.md` entry (bug-fix size): goal, the AST-corrected root strategy (default flip guard + handler-default
...
| No split error continuation           | AST re-scan                                       | adjacent `print_error*` + stdout `console.print` count 0        | AST meta guard                         |
