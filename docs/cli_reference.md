...
| `forge session adopt [conversation-id]` | Bind a Forge session to an existing native Claude conversation (`--name`, `--model`, `--yes`); bare lists unbound candidates (`--json`). Run from its launch directory |
