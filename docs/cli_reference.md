...
Codex runtime ([session design §3.9](design_sessions.md#39-session-resume-context-management)):
...
| `forge policy check --bundle <name> -f <path>`                 | Evaluate policies on demand; exactly one of `--file` / `-f` or `--diff` is required; multi-file diffs are evaluated per file, tests-first (`--json` includes `files_checked` and violation `file_path`) |
