...
| 3. Usage ledger | `usage/events/<YYYY-MM>_<pid>.jsonl`   | `core/usage/ledger.py:147` `log_usage_event`           | `UsageEvent` (run/parent/root, runtime, command, status, provider/model/proxy, `billing_mode`, `measurement_source`, tokens, latency, `cost_micro_usd`, `source_refs`) | **Attribution** (who/what/which runtime)           |
