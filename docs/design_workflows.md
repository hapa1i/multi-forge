...
3.8 Flash through OpenRouter (`google/gemini-3.8-flash`), local LiteLLM (`gemini/gemini-3.8-flash`), or remote LiteLLM
(`vertex_ai/gemini-3.8-flash`), configurable per provider via `--checker-provider`/`--checker-model`, with a
