...
Normative architecture for Forge policy, skills, and workflow runners. Core state lives in [design.md](design.md);
session, runtime, telemetry, installation, and memory contracts are routed by the [design map](design.md); command
inventories live in [cli_reference.md](cli_reference.md).
...
3.8 Flash (`google/gemini-3.8-flash`) through OpenRouter and Gemini 3.7 Flash through local LiteLLM
(`gemini/gemini-3.7-flash`) or remote LiteLLM (`vertex_ai/gemini-3.7-flash`), configurable per provider via
`--checker-provider`/`--checker-model`, with a configurable default prompt budget of roughly 32K tokens stored as
`policy.supervisor.checker_budget_tokens`) evaluates the action against the **approved-plan snapshot text**
...
Reference details for [Policy and Enforcement](#1-policy-enforcement).
