...
```bash
forge auth login [--credential NAME] [--profile PROFILE]
```
...
```bash
forge auth status [--profile PROFILE]
```
...
```bash
forge auth logout [--profile PROFILE] [--yes]
```
...
```bash
forge auth logout                     # Remove default profile (with confirmation)
forge auth logout --profile work -y   # Remove work profile, skip confirmation
```
...
```yaml
# Forge Credential Store — managed by `forge auth login`
...
**Do not edit this file by hand** unless you know what you're doing. Use `forge auth login` to manage it. The file uses
atomic writes and advisory locking to prevent corruption from concurrent access.
...
If `forge auth` commands fail with a parse error:
