...
empty. Inline emoji removal preserves surrounding word boundaries, including when the emoji touches text on one side.
The hook changes the pending message file and exits successfully, so Git records only the normalized form.
