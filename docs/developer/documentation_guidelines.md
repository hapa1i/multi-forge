...
Markdown policy is Opus-authoritative, with local tiktoken used as a conservative screen:
...
./scripts/check-file-limits.py --all-files
./scripts/check-file-limits.py --refresh-token-cache docs/design.md
```

The default counter reports both the method and tokenizer family that produced a count; human-readable method wording is
never parsed as policy. For provider-backed prose, the checker batches one local pass and accepts files at or below the
configured tiktoken target as an empirically conservative screen. Files above that screen require a current entry in the
tracked `.file-token-counts.json`, keyed by exact file SHA-256 and generated by Claude Opus 5. A missing, stale,
malformed, or method-mismatched entry is a hard error. A provider call that falls back to tiktoken cannot decide the
Opus target or ceiling.

Refresh the cache explicitly after changing a file that crosses its local screen, then stage both the document and
cache. Steady-state and keyless CI checks read the same cached provider result, so ambient credentials cannot reverse a
verdict. If the local tokenizer itself cannot run, the checker fails instead of silently skipping token policy.
Source-code rules remain local-only and compare tiktoken counts directly with their configured ceilings.
